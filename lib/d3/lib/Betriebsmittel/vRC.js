...
vRC.prototype.fhuelle = function () {
  this.Symbol.select("#huelle").append("rect").attr("fill", "none").attr("style", "stroke:#ffffff; stroke-width: 1.0").attr("x", this.data.Parameter.x - 20).attr("y", this.data.Parameter.y - 10).attr("height", this.data.vis.height + 20).attr("width", this.data.vis.width + 40).attr("hidden", "true"); // soll es sichtbar sein --> Zeile ausdokumentieren
};
...
vRC.prototype.fanschluss = function () {
  this.Symbol.select("#anschluss").append("path").attr("fill", this.data.vis.fill).attr("fill-opacity", this.data.vis.fillopacity).attr("style", this.data.vis.style).attr("d", this.data.vis.path);
};
vRC.prototype.fname = function () {
  this.Symbol.select("#name").append("text").attr("x", this.data.Parameter.x).attr("y", this.data.Parameter.y - 5).text(d => {
    return this.data.Parameter.Name;
  });
};
vRC.prototype.fvalue = function () {
  this.Symbol.select("#value").append("text").attr("x", this.data.Parameter.x + 50).attr("y", this.data.Parameter.y - 5).text(this.data.Parameter.R);
};
