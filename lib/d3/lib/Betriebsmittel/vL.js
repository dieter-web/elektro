function vL(data, element, pos) {
  ...
  // Anschlüsse
  Symbol.selectAll("path").data(element).enter().append("path").attr("fill", d => {
    return data.visMehrlagigeSpule.fill;
  }).attr("fill-opacity", d => {
    return data.visMehrlagigeSpule.fillopacity;
  }).attr("style", d => {
  ...
  // Layer Name
  //
  Name.selectAll("#name").data(element).enter().append("text").attr("x", pos[0]).attr("y", pos[1] - 5).text(d => {
  ...
  // Layer Value
  //
  Value.selectAll("#value").data(element).enter().append("text").attr("x", pos[0] + 50).attr("y", pos[1] - 5).text(d => {
