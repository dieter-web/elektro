function init() {
  ...
  cursor = svg.append("circle").attr("r", 10).attr("stroke", "black").attr("stroke-width", 1).attr("fill", "white").attr("transformation", "translate(-100, -100").attr("class", "cursor");
}
...
function doit(data, index) {
  ...
  if (data) {
    ...
    svg.append("circle").on("mousedown", doit).attr("cx", point[0]).attr("cy", point[1]).attr("r", 5).datum(42);
  }
}