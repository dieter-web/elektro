class vComponents {
  ...
  fvalue() {
    this.Symbol.select("#value").append("text").attr("x", this.point.x + 50).attr("y", this.point.y + 50).text(`${this.data.Parameter.erg}`);
    this.Symbol.select("#name").append("text").attr("x", this.point.x).attr("y", this.point.y + 50).text(`${this.data.Kennzeichnung.Art} ${this.data.Kennzeichnung.Zählnummer}`);
  }
  createNode() {
    this.finit();
    this.fshape();
