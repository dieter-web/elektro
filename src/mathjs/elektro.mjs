...
export class Elektro extends Kernel {
  ...
  registerFormulas(allowed) {
    const definitions = {
      ...
      // maximale Stromdichte R = Radius !
      SIR: { expr: 'I / (2 * PI * pow(R,2))', params: ['I', 'R'] },

      IρAlU: { expr: 'U * (A / (ρ * l))', params: ['U', 'A', 'ρ', 'l'] },

      ΔI: {
        expr: '(1 - ((1 + α20 * (δ1 - δ20)) / (1 + α20 * (δ2 - δ20)))) * 100',
        params: ['α20', 'δ1', 'δ20', 'δ2'],
      },
      ...
      RaUR1R2Ia: {
        expr: '( R2 / Ia ) * ( ( ( U - ( Ia * R1 ) ) / ( R1 + R2 ) ) + Ia ) - R2',
        params: ['U', 'R1', 'R2', 'Ia'],
      },

      ρRAl: { expr: '(R * A)/ l', params: ['R', 'A', 'l'] },

      // Widerstand eines Kegelstumpfs:
      Rρhr1r2: {
        expr: '(ρ * h / (PI * r1 * r2))',
        params: ['ρ', 'h', 'r1', 'r2'],
      },
      ...
      δ: {
        expr: '(((Rδ / R20) - 1) / α20) + δ20 ',
        params: ['Rδ', 'R20', 'α20', 'δ20'],
      },

      δ0: { expr: '(1 / α20) - δ20', params: ['α20', 'δ20'] },

      δ2: {
        expr: '(((Rδ2 - R) / R) * (δ0 + δ1)) + δ1',
        params: ['Rδ2', 'R', 'δ0', 'δ1'],
      },

      Rδ: {
        expr: 'R20 * ( 1 + α20 * (δ - δ20))',
        params: ['R20', 'α20', 'δ', 'δ20'],
      },
      ...
      RR1δ1δ2α20β20: {
        expr: 'R1 * (1 + α20 * ( δ2 - δ1 ) + β20 *(( δ2 - δ1 )*( δ2 - δ1 )))',
        params: ['R1', 'α20', 'β20', 'δ1', 'δ2'],
      },
      ...
      α20: {
        expr: '( (Rδ / R20) - 1 ) * (1 / (δ - δ20) )',
        params: ['Rδ', 'R20', 'δ', 'δ20'],
      },

      // Dreieck-Stern-Transformation
      R1s: {
        expr: 'R2d * R3d / (R1d + R2d + R3d)',
        params: ['R1d', 'R2d', 'R3d'],
      },
      R2s: {
        expr: 'R3d * R1d / (R1d + R2d + R3d)',
        params: ['R1d', 'R2d', 'R3d'],
      },
      R3s: {
        expr: 'R1d * R2d / (R1d + R2d + R3d)',
        params: ['R1d', 'R2d', 'R3d'],
      },
    ...
    definitions.ICombined = {
      ...
      params: [
        ...new Set([...definitions.IQt.params, ...definitions.UIR.params]),
      ],
    };
  ...
  bestimmeEinheit(parameter, formula) {
    const erg = this.berechne(parameter, formula);
    return erg.units[0].unit.name;
  }
